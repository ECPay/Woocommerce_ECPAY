...
class Wooecpay_Logistic_Response extends WC_Payment_Gateway
{
    ...
    public function map_response()
    {
        if(isset($_POST['MerchantTradeNo'])){
            ...
            if ($order = wc_get_order($order_id)){
                ...
                $order->add_order_note(sprintf(__('Ecpay Payment Merchant Trade No %s', 'wooecpay'), $merchant_trade_no));
        ...
        if(isset($_POST['MerchantTradeNo'])){
            ...
            if ($order = wc_get_order($order_id)){
                ...
                if(
                    ...
                    $order->add_order_note(sprintf(__('Change store %1$s (%2$s)', 'wooecpay'),$_POST['CVSStoreName'],$_POST['CVSStoreID']));
                ...
                echo '<tr>';
                echo '<td>超商店舖編號:</td>';
                echo '<td>'. $_POST['CVSStoreID'].'</td>';
                echo '</tr>';
                echo '<tr>';
                echo '<td>超商店舖名稱:</td>';
                echo '<td>'. $_POST['CVSStoreName'].'</td>';
                echo '</tr>';
                echo '<tr>';
                echo '<td>超商店舖地址:</td>';
                echo '<td>'. $_POST['CVSAddress'].'</td>';
                echo '</tr>';
                echo '</tbody>';
                echo '</table>';
                echo '</section>';
            ...
            if(isset($_POST['MerchantTradeNo'])){
                ...
                if ($order = wc_get_order($order_id)){
                    ...
                    if(
                        ...
                        $order->add_order_note('物流貨態回傳:'.$_POST['RtnMsg'].' ('.$_POST['RtnCode'].')');
                        $order->save();
                    }
                }
            }
    ...
    protected function add_type_info($input, $order)
    {
        ...
        switch ($payment_type) { 
            ...
            if (in_array($number_of_periods, [3, 6, 12, 18, 24, 30])) {
                ...
                $order->add_order_note(sprintf(__('Credit installment to %d', 'wooecpay'),$number_of_periods));
